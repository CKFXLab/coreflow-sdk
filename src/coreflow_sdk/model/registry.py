...
class ModelRegistry:
    ...
    def search_models(self, query: str, provider: Union[Provider, str] = None) -> List[ModelInfo]:
        """Search models by name, description, or alias."""
        query = query.lower()
        results = []
        
        models = self.get_models_by_provider(provider) if provider else list(self._models.values())
        ...
        for model in models:
            if (query in model.model_id.lower() or 
                ...
                any(query in alias.lower() for alias in model.aliases)):
    ...
    def get_available_models_by_credentials(self, env=None) -> Dict[str, Any]:
        ...
        for provider in Provider:
            ...
            if provider == Provider.OPENAI:
                ...
                provider_available = available_credentials.get('anthropic_api_key', False)
            elif provider == Provider.BEDROCK:
                # AWS can work with multiple credential methods
                provider_available = (
                    available_credentials.get('aws_access_key_id', False) or
                    available_credentials.get('aws_profile_available', False) or
                    available_credentials.get('aws_instance_profile', False)
                )
            ...
            providers[provider.value] = {
                ...
                "credential_status": self._get_provider_credential_status(provider, available_credentials)
            }
    ...
    def _get_provider_credential_status(self, provider: Provider, credentials: Dict[str, bool]) -> Dict[str, Any]:
        """Get detailed credential status for a provider."""
        if provider == Provider.OPENAI:
            return {
                "required": ["OPENAI_API_KEY"],
                "available": credentials.get('openai_api_key', False),
                "missing": [] if credentials.get('openai_api_key', False) else ["OPENAI_API_KEY"]
            }
        elif provider == Provider.ANTHROPIC:
            return {
                "required": ["ANTHROPIC_API_KEY"],
                "available": credentials.get('anthropic_api_key', False),
                "missing": [] if credentials.get('anthropic_api_key', False) else ["ANTHROPIC_API_KEY"]
            }
        elif provider == Provider.BEDROCK:
            aws_available = (
            ...
            if not aws_available:
                missing = ["AWS_ACCESS_KEY_ID + AWS_SECRET_ACCESS_KEY", "AWS_PROFILE", "IAM_ROLE"]
    ...
    def get_available_providers_list(self, env=None) -> List[str]:
        ...
        if (credentials.get('aws_access_key_id', False) or 
            credentials.get('aws_profile_available', False) or 
            credentials.get('aws_instance_profile', False)):
