...
class OpenAIClient(APIModel):
    ...
    def __init__(self, api_key: str = None, base_url: str = None, timeout: int = 30, 
                 model: str = "gpt-4", model_type: str = "chat", **kwargs):
        """
        ...
        if not self.api_key:
            self.logger.error("OpenAI API key not provided and OPENAI_API_KEY environment variable not set")
    ...
    def forward(self, prompt: str = None, messages: List[Dict[str, str]] = None, **kwargs) -> Dict[str, Any]:
        ...
        request_kwargs = {k: v for k, v in request_kwargs.items() if k not in client_params}
        ...
        for attempt in range(self.num_retries + 1):
            try:
                ...
                self.update_global_history(history_entry)  # Uses inherited implementation
                ...
                if attempt < self.num_retries:
                    ...
                    self.logger.error(f"OpenAI request failed after {self.num_retries} retries: {e}")
                    raise
        ...
        for attempt in range(self.num_retries + 1):
            try:
                ...
                if attempt < self.num_retries:
                    ...
                    self.logger.error(f"OpenAI async request failed after {self.num_retries} retries: {e}")
                    raise
    ...
    def finetune(self, train_data: List[Dict[str, Any]], train_data_format: Optional[str] = None,
                train_kwargs: Optional[Dict[str, Any]] = None) -> 'OpenAITrainingJob':
    ...
    def validate_connection(self) -> bool:
        ...
        try:
            ...
            if test_response and test_response.data:
                ...
                self.logger.error("OpenAI connection validation failed - no data returned")
                return False
    ...
    def _process_response(self, response: Dict[str, Any], prompt: str, messages: List[Dict[str, str]], **kwargs) -> Any:
