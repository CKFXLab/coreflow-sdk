...
class BedrockAnthropicClient(BedrockModel):
    ...
    def _setup_boto3_clients(self) -> None:
        ...
        try:
            ...
            if self.aws_access_key_id and self.aws_secret_access_key:
                ...
                self.logger.info("Using AWS credential chain (environment, profile, IAM role, etc.)")
    ...
    def forward(self, prompt: str = None, messages: List[Dict[str, str]] = None, **kwargs) -> Dict[str, Any]:
        ...
        bedrock_payload = self._prepare_bedrock_payload(messages or [{"role": "user", "content": prompt}], **kwargs)
    ...
    def _prepare_bedrock_payload(self, messages: List[Dict[str, str]], **kwargs) -> Dict[str, Any]:
        """
    ...
    def inspect_history(self, n: int = 1) -> str:
        ...
        for i, entry in enumerate(recent_entries, 1):
            formatted_entry = f"--- Bedrock Translation Entry {len(self.history) - n + i} ---\n"
            formatted_entry += f"DSPy Input: {entry.get('messages', entry.get('prompt'))}\n"
    ...
    def get_model_invoke_cost(self, model_id: str, input_tokens: int, output_tokens: int) -> float:
    ...
    def finetune(self, train_data: List[Dict[str, Any]], train_data_format: Optional[str] = None,
                train_kwargs: Optional[Dict[str, Any]] = None) -> BedrockTrainingJob:
    ...
    def generate_embedding(self, text: str, model: str = None) -> List[float]:
        ...
        try:
            ...
            self.logger.error(f"Failed to generate embedding with model {embedding_model}: {e}")
            raise
