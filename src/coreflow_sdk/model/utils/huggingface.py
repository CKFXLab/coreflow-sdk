...
class HuggingFace:
    ...
    def __init__(self, cache_dir: Optional[str] = None, token: Optional[str] = None):
        ...
        self.logger.info(
            f"HuggingFace SDK initialized with cache dir: {self.cache_dir}"
        )
        ...
        try:
            ...
            for model in models:
                model_info = ModelInfo(
                    ...
                    model_type=getattr(model, "config", {}).get(
                        "model_type", "unknown"
                    ),
            ...
            self.logger.info(
                f"Found {len(model_infos)} models matching search criteria"
            )
            return model_infos
    ...
    def download_file(
        ...
        try:
            ...
            self.logger.info(
                f"Downloaded file {filename} from {model_id} to: {file_path}"
            )
            return file_path

        except Exception as e:
            self.logger.error(
                f"Failed to download file {filename} from {model_id}: {e}"
            )
            raise
    ...
    def load_model(self, model_id: str, **kwargs) -> Any:
        ...
        try:
            ...
            model = AutoModel.from_pretrained(
                model_id, cache_dir=str(self.cache_dir), token=self.token, **kwargs
            )
    ...
    def create_pipeline(
        self, task: str, model_id: Optional[str] = None, **kwargs
    ) -> Any:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Creating {task} pipeline with model: {model_id or 'default'}"
            )

            pipe = pipeline(task=task, model=model_id, token=self.token, **kwargs)
