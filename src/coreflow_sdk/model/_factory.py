...
class ModelFactory:
    ...
    def create_model(
        self, model_config: Union[Dict[str, Any], Model], **override_kwargs
    ) -> Model:
        """
        ...
        try:
            ...
            if isinstance(model_config, Model):
                self.logger.info(
                    f"Using pre-instantiated model: {model_config.__class__.__name__}"
                )
                return model_config
    ...
    def _prepare_init_kwargs(
        ...
        if provider == "openai":
            ...
            if "model" in kwargs:
                kwargs["model_id"] = kwargs.pop(
                    "model"
                )  # Bedrock uses model_id parameter
    ...
    def create_default_model(self, provider: Optional[str] = None) -> Model:
        ...
        config = {"provider": provider, "model": DEFAULT_MODELS[provider]}
