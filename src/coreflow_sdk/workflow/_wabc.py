...
class BaseWorkflow:
    ...
    def __init__(
        ...
        if self.disabled_features:
            self.logger.warning(
                f"Disabled features due to missing credentials: {', '.join(self.disabled_features)}"
            )

        # Auto-adjust feature flags based on credentials
        enable_websearch = enable_websearch and self.available_credentials.get(
            "serper_api_key", False
        )
        if (
            not self.available_credentials.get("serper_api_key", False)
            and enable_websearch
        ):
        ...
        if model_config is None:
            ...
            if model_config is None:
                ...
                raise ValueError(
                    "No model provider credentials available. Please set OPENAI_API_KEY, ANTHROPIC_API_KEY, or AWS credentials."
                )
            else:
                self.logger.info(
                    f"Using best available model: {model_config['provider']}:{model_config['model']}"
                )
    ...
    def _init_memory_client(self):
        ...
        try:
            ...
            if self.available_credentials.get("mem0_api_key", False):
                ...
                self.logger.info(
                    "Memory client initialized with local fallback (MEM0_API_KEY not available)"
                )
    ...
    def _init_vector_client(self):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to initialize vector client with environment config: {e}"
            )
            try:
                ...
                self.logger.error(
                    f"Failed to initialize vector client: {fallback_error}"
                )
                self._component_status["vector_client"] = False
                self.vector_client = None
    ...
    def _init_web_clients(self):
        ...
        try:
            ...
            if not serper_key:
                self.logger.warning(
                    "SERPER_API_KEY not available - web search disabled"
                )
                self.search_client = None
                self._component_status["search_client"] = False
            else:
    ...
    def process_query(self, query: str, user_id: str = "default_user") -> str:
        ...
        try:
            ...
            instruction = (
                WEB_DATA_INSTRUCTION
                if context.get("has_web_data", False)
                else DEFAULT_INSTRUCTION
            )

            # Use standardized prompt structure from _default.py
            prompt = (
                PROMPT_STRUCTURE.format(
                    agent_role=AGENT_ROLE, query=query, context=context_text
                )
                + f"\n{instruction}"
            )
    ...
    def generate_response(
        ...
        try:
            ...
            model_response = self.model_client(
                messages=messages, temperature=0.7, **kwargs
            )
    ...
    def _gather_vector_context(
        self, query: str, collection_type: str = "user", user_id: str = "default_user"
    ) -> str:
        """
        ...
        if not self.enable_rag or not self.vector_client:
            return (
                "RAG disabled" if not self.enable_rag else "No vector context available"
            )

        try:
            if collection_type == "user":
                f"{self.user_collection_prefix}{user_id}"
            else:
    ...
    def _gather_web_context(self, query: str, max_results: int = 3) -> Dict[str, str]:
        ...
        if not self._should_trigger_web_search(query):
            context["search_results"] = (
                "No time-sensitive keywords detected - using knowledge base"
            )
    ...
    def _store_memory(
        self, query: str, response: str, user_id: str, memory_type: str = "user"
    ) -> bool:
        """
        ...
        try:
            if memory_type == "system":
                return self.memory_client.store_system_memory(
                    f"Q: {query} | A: {response}"
                )
            else:
    ...
    def validate_workflow(self) -> Dict[str, Any]:
        ...
        if not self.model_client:
            ...
            validation["errors"].append(
                "Model client not initialized - workflow cannot generate responses"
            )

        # Check optional components and add warnings for missing credentials
        if not self.memory_client:
            validation["warnings"].append(
                "Memory client not available - no conversation history"
            )

        if not self.vector_client:
            validation["warnings"].append(
                "Vector client not available - no document retrieval"
            )

        if not self.search_client:
            if not self.available_credentials.get("serper_api_key", False):
                validation["warnings"].append(
                    "Search client not available - SERPER_API_KEY missing"
                )
            else:
                validation["warnings"].append(
                    "Search client not available - initialization failed"
                )

        if not self.scrape_client:
            validation["warnings"].append(
                "Scrape client not available - no webpage content"
            )
    ...
    def _gather_context(self, query: str, user_id: str) -> Dict[str, Any]:
        ...
        context = {
            ...
            "has_web_data": (
                self.enable_websearch
                and web_context.get("search_results")
                not in [
                    "Web search disabled",
                    "No time-sensitive keywords detected - using knowledge base",
                ]
    ...
    def _build_web_section(self, web_context: Dict[str, str]) -> str:
        ...
        """
        search_results = web_context.get(
            "search_results", "No search results available"
        )
        webpage_content = web_context.get(
            "webpage_content", "No webpage content available"
        )

        # Only include web section if we have actual search results (not disabled or keyword-filtered)
        if self.enable_websearch and search_results not in [
            "Web search disabled",
            "No time-sensitive keywords detected - using knowledge base",
        ]:
    ...
    def _store_interaction(self, query: str, response: str, user_id: str):
        """Store interaction in both memory and vector storage."""
        # Store in memory
    ...
    async def stream_response(
        ...
        """
        Stream workflow response in real-time.
        ...
        Yields:
            Dict with streaming data: {"type": "chunk|context|complete|error", "data": ...}
        """
        try:
            # Yield start event
            yield {"type": "start", "data": {"query": query, "user_id": user_id}}

            # Gather context (non-streaming for now)
            context = self._gather_context(query, user_id)
            yield {"type": "context", "data": {"context_gathered": True}}

            # Build prompt like regular workflow
            formatted_context = self._format_context_for_streaming(context)
            ...
            if hasattr(self.model_client, "stream_forward"):
                # Model supports streaming
                try:
                    stream_result = self.model_client.stream_forward(
                        prompt=prompt, **model_kwargs
                    )

                    # Handle both real async generators and mocked coroutines
                    if hasattr(stream_result, "__aiter__"):
                        ...
                        # Mocked coroutine - await it first
                        async_gen = await stream_result
                ...
                response = await self.model_client.aforward(
                    prompt=prompt, **model_kwargs
                )
                content = self._process_response(response, query, None, **model_kwargs)
                full_response = content
                yield {"type": "chunk", "data": {"content": content}}

            # Process and store like regular workflow
            processed_response = self.process_response(full_response, query, user_id)
            self._store_interaction(query, processed_response, user_id)

            # Yield completion
            yield {"type": "complete", "data": {"response": processed_response}}

        except Exception as e:
            yield {"type": "error", "data": {"error": str(e)}}

    def _format_context_for_streaming(self, context: Dict[str, Any]) -> str:
        """Format context for streaming (same as regular workflow)."""
        formatted_sections = []

        if context.get("memory_context"):
            formatted_sections.append(f"Memory Context:\n{context['memory_context']}")

        if context.get("vector_context"):
            formatted_sections.append(f"Knowledge Base:\n{context['vector_context']}")

        if context.get("web_context"):
            web_section = self._build_web_section(context["web_context"])
            formatted_sections.append(web_section)

        return "\n\n".join(formatted_sections)

    def supports_streaming(self) -> bool:
        """Check if current model supports streaming."""
        return hasattr(self.model_client, "stream_forward")
