...
class CustomWorkflow:
    ...
    def __init__(
        self, base_workflow, method_overrides: Optional[Dict[str, Callable]] = None
    ):
        """
    ...
    def _bind_override_method(
        self, override_func: Callable, method_name: str
    ) -> Callable:
        """
...
class WorkflowFactory:
    ...
    def create_workflow(
        self, workflow_config: Union[Dict[str, Any], Any], **override_kwargs
    ) -> Any:
        """
        ...
        try:
            ...
            if hasattr(
                workflow_config, "generate_response"
            ):  # Duck typing for workflow
                self.logger.info(
                    f"Using pre-instantiated workflow: {workflow_config.__class__.__name__}"
                )
            ...
            if not isinstance(workflow_config, dict):
                raise ValueError(
                    f"workflow_config must be a dictionary, got {type(workflow_config)}"
                )
            ...
            self.logger.error(
                f"Failed to create workflow from config {workflow_config}: {e}"
            )
            raise
    ...
    def _validate_config(self, config: Dict[str, Any]) -> None:
        ...
        if workflow_type not in WORKFLOW_CLASS_MAP:
            ...
            raise ValueError(
                f"Unsupported workflow type: {workflow_type}. Available: {available}"
            )
    ...
    def _validate_method_overrides(self, method_overrides: Dict[str, Any]) -> None:
        ...
        for method_name, override_func in method_overrides.items():
            if method_name not in valid_methods:
                raise ValueError(
                    f"Invalid method override: {method_name}. Valid methods: {valid_methods}"
                )

            if not callable(override_func):
                raise ValueError(
                    f"Method override for {method_name} must be callable, got {type(override_func)}"
                )

    def _validate_workflow_specific_config(
        self, config: Dict[str, Any], workflow_type: str
    ) -> None:
        """
    ...
    def _validate_multi_agent_config(self, config: Dict[str, Any]) -> None:
        ...
        if strategy not in COORDINATION_STRATEGIES:
            ...
            raise ValueError(
                f"Invalid coordination strategy: {strategy}. Available: {available}"
            )
    ...
    def _validate_custom_config(self, config: Dict[str, Any]) -> None:
        ...
        for i, component in enumerate(custom_components):
            if not isinstance(component, dict):
                raise ValueError(
                    f"Custom component {i} configuration must be a dictionary"
                )
    ...
    def _instantiate_workflow(self, config: Dict[str, Any]) -> Any:
        ...
        if method_overrides:
            self.logger.info(
                f"Applying method overrides: {list(method_overrides.keys())}"
            )
    ...
    def _get_workflow_class(self, workflow_type: str):
        ...
        if workflow_type not in WORKFLOW_IMPORT_PATHS:
            raise ValueError(
                f"No import path configured for workflow type: {workflow_type}"
            )
        ...
        try:
            ...
            raise ImportError(
                f"Failed to import {workflow_type} workflow. "
                f"Make sure dependencies are installed: {e}"
            )
        except AttributeError as e:
            raise ImportError(
                f"Workflow class {class_name} not found in {module_path}: {e}"
            )
    ...
    def create_default_workflow(self, workflow_type: Optional[str] = None) -> Any:
        ...
        if workflow_type not in DEFAULT_WORKFLOW_CONFIGS:
            raise ValueError(
                f"No default configuration for workflow type: {workflow_type}"
            )
