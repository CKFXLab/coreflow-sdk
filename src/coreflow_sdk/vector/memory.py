...
from ._default import CollectionType, get_config
...
class Mem0:
    ...
    def __init__(self, config: dict = None, env: Optional[ENV] = None):
        ...
        try:
            ...
            model = (
                self.config.get("embedder", {})
                .get("config", {})
                .get("model", "unknown")
            )
            self.logger.info(
                f"Mem0 client initialized with {provider} embeddings (model: {model})"
            )
    ...
    def store_memory(
        ...
        memory_type: str = CollectionType.USER,
    ) -> bool:
        """
        ...
        try:
            ...
            target_user_id = (
                self.SYSTEM_USER_ID if memory_type == CollectionType.SYSTEM else user_id
            )
            ...
            if memory_type == CollectionType.SYSTEM:
                ...
                self.logger.info("Stored system memory")
            else:
        ...
        try:
            ...
            if memories:
                ...
                if context_parts:
                    ...
                    self.logger.debug(
                        f"Retrieved {len(context_parts)} memory context items from {memory_source}"
                    )
                    return result
                else:
    ...
    def get_system_memory_context(self, query: str, limit: int = 5) -> str:
        ...
        return self.get_memory_context(
            query, memory_type=CollectionType.SYSTEM, limit=limit
        )
    ...
    def store_system_conversation(self, query: str, response: str) -> bool:
        ...
        return self.store_conversation(
            query, response, memory_type=CollectionType.SYSTEM
        )

    def get_all_memories(
        self, user_id: str = "default_user", memory_type: str = CollectionType.USER
    ) -> List[Dict]:
        """
