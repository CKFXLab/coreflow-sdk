...
from ._default import (
    COLLECTION_NAME, USE_DOCKER_QDRANT, QDRANT_HOST, QDRANT_PORT,
    SYSTEM_COLLECTION_NAME, USER_COLLECTION_PREFIX, CollectionType,
    create_qdrant_config, create_embedding_client, get_embedding_client_config
)
from ..utils.audit import AppLogger
from ..utils.env import ENV
from .utils.file_operations import FileOperations, FileInfo, TextChunk
from .utils.collections import create_user_collection_name, sanitize_username
...
class RAG:
    ...
    def __init__(self, collection_name: str = COLLECTION_NAME, purge: bool = True, 
                 use_docker_qdrant: bool = None, qdrant_host: str = None, 
                 qdrant_port: int = None, env: Optional[ENV] = None):
        """
    ...
    def _init_embedding_client(self):
        ...
        try:
            ...
            self.logger.info(f"Embedding client initialized: {provider} with model {model}")
        except Exception as e:
            ...
            try:
                ...
                self.logger.error(f"Failed to initialize fallback embedding client: {fallback_error}")
                raise
        ...
        try:
            ...
            if qdrant_url:
                ...
                if api_key:
                    ...
                    self.logger.info(f"Connected to Qdrant at {qdrant_url} with API key")
                else:
            ...
            # Host/port-based connection (for local Docker or direct connection)
                ...
                self.logger.info(f"Connected to Docker Qdrant at {self.qdrant_host}:{self.qdrant_port}")
                return True
    ...
    def _ensure_collection_exists(self, collection_name: str, embedding_dims: int = 1536) -> None:
        ...
        try:
            if self.client.collection_exists(collection_name):
                ...
                self.logger.info(f"Created collection {collection_name} with {embedding_dims} dimensions")
        except Exception as e:
            self.logger.error(f"Failed to ensure collection {collection_name} exists: {e}")

    def _get_collection_name(self, collection_type: str, user_id: Optional[str] = None) -> str:
    ...
    def get_collection_info(self, collection_type: str = CollectionType.SYSTEM, user_id: Optional[str] = None):
        ...
        try:
            if self.client.collection_exists(collection_name):
                ...
                self.logger.info(f"Collection {collection_name} exists with {info.points_count} points")
                return info
            else:
    ...
    def search_similar(self, query: str, collection_type: str = CollectionType.SYSTEM, 
        ...
        try:
            ...
            query_embedding = self.embedding_client.generate_embedding(query, embedding_model)
            ...
            self.logger.info(f"Found {len(search_results)} similar results in {collection_name}")
    ...
    def search_across_collections(self, query: str, user_id: Optional[str] = None, 
    ...
    def store(self, text: str, metadata: dict, collection_type: str = CollectionType.SYSTEM, 
        ...
        try:
            ...
            if "id" in metadata:
                enhanced_metadata["original_id"] = metadata["id"]  # Preserve original string ID
    ...
    def delete_user_collection(self, user_id: str):
        ...
        try:
            ...
            self.logger.error(f"Failed to delete user collection {collection_name}: {e}")
            return False
    ...
    def process_file(self, 
                    ...
                    chunk_size: int = 1000,
                    overlap: int = 200,
                    chunking_method: str = "sliding_window") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(f"Processing file: {file_info.filename} ({file_info.file_type}, {file_info.size_mb}MB)")
            ...
            try:
                ...
                if not content or not content.strip():
                    results['errors'].append("File content is empty or contains only whitespace")
                    return results
            ...
            base_metadata.update(self.file_ops.extract_metadata_from_content(content, filepath))
            ...
            self.logger.info(f"Created {len(chunks)} chunks using {chunking_method} method")
            ...
            for i, chunk in enumerate(chunks):
                try:
                    ...
                    if success:
                        ...
                        chunks_failed += 1
                        self.logger.warning(f"Failed to store chunk {i+1}/{len(chunks)}")
    ...
    def process_file_to_system(self, 
                              filepath: str,
                              subject: Optional[str] = None,
                              short_description: Optional[str] = None,
                              chunk_size: int = 1000,
    ...
    def batch_process_files(self, 
        ...
        for filepath in filepaths:
            try:
                ...
                if result['success']:
                    ...
                    batch_results['total_chunks_processed'] += result['chunks_processed']
                    batch_results['total_chunks_failed'] += result['chunks_failed']
                else:
                    batch_results['files_failed'] += 1
                    if not continue_on_error:
                        self.logger.error(f"Stopping batch processing due to failure with {filepath}")
                        break
        ...
        success_rate = (batch_results['files_processed'] / len(filepaths)) * 100 if filepaths else 0
        self.logger.info(
    ...
    def get_file_processing_stats(self, 
                                 collection_type: str = CollectionType.SYSTEM,
