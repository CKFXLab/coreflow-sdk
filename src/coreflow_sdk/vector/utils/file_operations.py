...
class FileOperations:
    ...
    def _log_dependency_status(self):
        ...
        if available:
            self.logger.info(f"Available file processing libraries: {', '.join(available)}")
        if unavailable:
            self.logger.warning(f"Unavailable file processing libraries: {', '.join(unavailable)}")
    ...
    def detect_file_type(self, filepath: str) -> str:
        ...
        # Fallback to text
        self.logger.warning(f"Could not determine file type for {filepath}, defaulting to text")
        return 'text'
            ...
            for alt_encoding in ['latin-1', 'cp1252', 'iso-8859-1']:
                try:
                    with open(filepath, 'r', encoding=alt_encoding) as f:
                        content = f.read()
                    self.logger.warning(f"Used fallback encoding {alt_encoding} for {filepath}")
                    return content
                except UnicodeDecodeError:
                    continue
    ...
    def read_csv_file(self, filepath: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            self.logger.debug(f"Read CSV file: {filepath} ({len(df)} rows, {len(df.columns)} columns)")
            return df
        except Exception as e:
            ...
            with open(filepath, 'rb') as f:
                ...
                for page_num, page in enumerate(pdf_reader.pages):
                    try:
                        ...
                        self.logger.warning(f"Failed to extract text from page {page_num} in {filepath}: {e}")
            
            content = '\n\n'.join(text_content)
            self.logger.debug(f"Read PDF file: {filepath} ({len(pdf_reader.pages)} pages, {len(content)} characters)")
            return content
        except Exception as e:
    ...
    def read_xlsx_file(self, filepath: str, include_sheet_names: bool = True) -> str:
        ...
        try:
            ...
            content = '\n'.join(text_content)
            self.logger.debug(f"Read XLSX file: {filepath} ({len(excel_data)} sheets, {len(content)} characters)")
            return content
        except Exception as e:
    ...
    def read_html_file(self, filepath: str, extract_text_only: bool = True) -> str:
        ...
        try:
            ...
            if extract_text_only:
                ...
                self.logger.debug(f"Read HTML file: {filepath} (extracted {len(text)} characters)")
                return text
            else:
                self.logger.debug(f"Read HTML file: {filepath} ({len(content)} characters)")
                return content
        except Exception as e:
    ...
    def read_file(self, filepath: str, **kwargs) -> str:
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Failed to read file {filepath}: {e}. Fallback also failed: {fallback_e}")
    ...
    def chunk_text(self, 
        ...
        while start < len(text):
            ...
            if chunk_text:
                chunk_id = hashlib.md5(f"{source_file}_{chunk_index}_{start}_{end}".encode()).hexdigest()
    ...
    def chunk_by_paragraphs(self, 
        ...
        for paragraph in paragraphs:
            ...
            if current_chunk and len(current_chunk) + len(paragraph) + 2 > max_chunk_size:
                if current_chunk.strip():
                    ...
                    chunk_id = hashlib.md5(f"{source_file}_{chunk_index}_{start_char}_{end_char}".encode()).hexdigest()
        ...
        self.logger.debug(f"Created {len(chunks)} paragraph-based chunks from {len(paragraphs)} paragraphs")
        return chunks
    ...
    def is_supported_file(self, filepath: str) -> bool:
        ...
        return any(extension in extensions for extensions in self.supported_formats.values())
    ...
    def extract_metadata_from_content(self, content: str, filepath: str = None) -> Dict[str, Any]:
        """
        ...
        if content:
            ...
            if summary and not summary.endswith('.'):
                summary += '.'
            metadata['auto_summary'] = summary[:200] + '...' if len(summary) > 200 else summary
