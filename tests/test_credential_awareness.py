...
from coreflow_sdk.workflow._wabc import BaseWorkflow
from coreflow_sdk.workflow._default import (
...
class TestWorkflowCredentialAwareness:
    ...
    def test_workflow_auto_configuration(self):
        ...
        with patch.dict(
            os.environ,
            {"OPENAI_API_KEY": "test-openai-key", "SERPER_API_KEY": "test-serper-key"},
            clear=True,
        ):
    ...
    def test_workflow_no_model_credentials(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with pytest.raises(
                ValueError, match="No model provider credentials available"
            ):
                BaseWorkflow()
    ...
    def test_workflow_validation(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-openai-key"}, clear=True):
            ...
            with (
                ...
                assert any(
                    "Feature disabled" in warning for warning in validation["warnings"]
                )
...
class TestWorkflowDefaultFunctions:
    ...
    def test_get_available_features(self):
        ...
        with patch.dict(
            os.environ,
            {"OPENAI_API_KEY": "test-openai-key", "HF_TOKEN": "test-hf-token"},
            clear=True,
        ):
    ...
    def test_workflow_graceful_degradation(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-openai-key"}, clear=True):
            ...
            with (
                ...
                assert (
                    status["search_client"] is False
                )  # This should fail due to no SERPER_API_KEY
