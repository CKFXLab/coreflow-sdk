...
class TestFastAPIIntegration:
    ...
    def _add_chat_endpoints(self, app: FastAPI, workflow: Mock):
        ...
        async def chat_stream_endpoint(request: ChatRequest):
            ...
            async def generate_stream():
                try:
                    workflow.process_query(request.query, request.user_id)
        ...
        async def execute_workflow(workflow_id: str, request: WorkflowExecuteRequest):
            ...
            try:
                response = workflow.generate_response(
                    query=request.query, user_id=request.user_id, **request.parameters
                )
    ...
    def _add_rag_endpoints(self, app: FastAPI):
        ...
        async def list_collections(user_id: str = "default_user"):
            try:
                ...
                user_collections = [
                    {"name": f"user_{user_id}_docs", "type": "user", "user_id": user_id}
                ]
                system_collections = [{"name": "system_docs", "type": "system"}]
        ...
        async def store_to_collection(
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to store documents: {str(e)}"
                )

        @app.post("/rag/collections/{collection_name}/upload")
        async def upload_file_to_collection(
            collection_name: str, file: UploadFile, user_id: str = "default_user"
        ):
            try:
    ...
    def _add_model_endpoints(self, app: FastAPI):
        ...
        @app.post("/models/{model_name}/health")
            ...
            try:
                ...
                return {
                    ...
                    "components": {
                        ...
                        "vector": {"available": component_status["vector_client"]},
                        "search": {"available": component_status["search_client"]},
                        "scrape": {"available": component_status["scrape_client"]},
                    },
                    "validation": validation,
                }
            except Exception as e:
        ...
        async def component_health():
            try:
                components = {
                    ...
                    "memory": {"status": "healthy", "stats": {"total_memories": 10}},
                    "vector": {"status": "healthy", "collections_count": 2},
                }

                return {"components": components, "timestamp": time.time()}
            except Exception as e:
                raise HTTPException(
    ...
    def test_chat_endpoint_success(self, client):
        ...
        response = client.post(
            "/chat", json={"query": "What is machine learning?", "user_id": "test_user"}
        )
    ...
    def test_chat_stream_endpoint(self, client):
        ...
        response = client.post(
            "/chat/stream", json={"query": "Stream test", "user_id": "test_user"}
        )
    ...
    def test_execute_workflow_success(self, client):
        ...
        response = client.post(
            "/workflows/single_agent/execute",
            json={
    ...
    def test_execute_workflow_default_params(self, client):
        ...
        response = client.post(
            "/workflows/single_agent/execute", json={"query": "Test execution"}
        )
    ...
    def test_store_to_collection(self, client):
        ...
        response = client.post(
            "/rag/collections/test_collection/store",
            json={
    ...
    def test_upload_file_to_collection(self, client):
        ...
        response = client.post(
            "/rag/collections/test_collection/upload",
    ...
    def test_search_collection(self, client):
        ...
        response = client.get(
            "/rag/collections/test_collection/search",
            params={"query": "test query", "user_id": "test_user", "limit": 5},
        )
    ...
    def test_healthcheck_model(self, client):
        ...
        response = client.post("/models/gpt-4o-mini/health")
    ...
    def test_store_document_missing_text(self, client):
        ...
        response = client.post(
            "/rag/collections/test/store", json={"metadata": {"source": "test"}}
        )
    ...
    def test_workflow_execute_missing_query(self, client):
        ...
        response = client.post(
            "/workflows/single_agent/execute", json={"user_id": "test_user"}
        )
